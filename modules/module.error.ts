/**
 * module.error.ts — readable network/error diagnostics.
 *
 * Node's fetch (undici) reports almost every transport problem as the same
 * opaque `TypeError: fetch failed`, hiding the real reason inside a nested
 * `cause` chain. These helpers unwrap that chain and turn the common failure
 * codes into an actionable hint.
 */
