...
function buildWidevinePssh(kids: string[]): string {
  ...
  let o = 0;
  b.writeUInt32BE(size, o); o += 4;
  b.write('pssh', o, 'ascii'); o += 4;
  b.writeUInt32BE(0, o); o += 4; // version 0 + flags
  Buffer.from('edef8ba979d64acea3c827dcd51d21ed', 'hex').copy(b, o); o += 16;
  b.writeUInt32BE(data.length, o); o += 4;
  data.copy(b, o);
...
function buildV1Pssh(kids: string[]): string {
  ...
  b.writeUInt32BE(size, o); o += 4;
  b.write('pssh', o, 'ascii'); o += 4;
  b.writeUInt32BE(0x01000000, o); o += 4; // version 1
  Buffer.from('edef8ba979d64acea3c827dcd51d21ed', 'hex').copy(b, o); o += 16;
  b.writeUInt32BE(kids.length, o); o += 4;
  for (const k of kids) { Buffer.from(k, 'hex').copy(b, o); o += 16; }
  b.writeUInt32BE(0, o);
  return b.toString('base64');
}
...
(async () => {
  ...
  let licenceCalls = 0;
  const licence = async () => { licenceCalls++; return [{ kid: KID_A, key: KEY_A }, { kid: KID_B, key: KEY_B }]; };
  ...
  assert.deepStrictEqual(
    second.map((k) => normaliseKid(k.kid)).sort(),
    [KID_A, KID_B].sort()
  );
