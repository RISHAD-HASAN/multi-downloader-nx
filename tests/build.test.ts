...
assert.deepStrictEqual(
	missing,
	[],
	`modules/build.ts does not ship these config files (packaged builds would lose them): ${missing.join(', ')}`
);
...
assert.ok(
	/key_vaults:\s*\[\{\s*type:\s*'SQLite'/.test(cfgLoader),
	'module.cfg-loader.ts lost its default SQLite vault fallback'
);
...
assert.ok(
	vault.includes("require('node:sqlite')"),
	'module.vault.ts must use a literal require("node:sqlite") so esbuild leaves it external'
);
